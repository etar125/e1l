/*
Copyright (c) 2025-2026 etar125 Admanse
...
int sarr_insert(str_t *sarr, size_t at, str_t *jstr) {
    size_t count = sarr_count(sarr);
    if (count == 0 || at >= count || jstr->size == 0) { return 1; }
    char *str = sarr->data;
    size_t size = sarr->size;
    str_t new;
    if (at == 0) {
        ...
        part2.data = &str[i];
        part2.size = size - i;
        str_t new_part1 = join(&part1, jstr, '\n', false);
        if (!new_part1.data) { return 1; }
        new = join(&new_part1, &part2, '\n', false);
        free(new_part1.data);
        if (!new.data) { return 1; }
    }
    free(str);
